...
function processPdfData(pdfData) {
        ...
        let pageRes = e.Texts.map((t) => {
            ...
            let text = t.R.map((r) => {
                ...
                if (r.T === "%E2%86%90") {
                    ...
                    if (lastWasArrow === true) {
                        // Two arrows in a row is impossible, the second one is 100% bad
                        currLeft = "";
                    }
                    lastWasArrow = true;
                    ...
                    if (!addBracket) {
                        // This is like doing if braceOpened at the top because we set addBracket to the opposite of braceOpened
                        if (
                            prevLeft.trim().length >= 3 &&
                            currRight.trim().length >= 3
                        ) {
                            pageArticulations.push([
                                prevLeft,
                                currRight,
                                currArrowY,
                                pageNum,
                ...
                if (
                    r.TS.toString() !== [2, 23, 1, 0].toString() &&
                    r.TS[1] != 25
                )
                    return "";
                lastWasArrow = false;
                if (r.TS[1] === 25 && t.x > 16 && t.x < 22) {
                    if (
                        currExtraComboMarkerText === "---%20And%20--" ||
                        currExtraComboMarkerText === "---%20Or%20--"
                    ) {
                        pageComboMarkers.push([
                            currExtraComboMarkerText.replaceAll("%20", " ") +
                                "-",
                            currExtraComboMarkerY,
                        ]);
                    }
            ...
            if (c[0] === "--- And ---") {
                ...
                arrowYs[i].forEach((a) => {
                    if (a > c[1]) {
                        ...
                        if (prevArrowY !== 0)
                            removeFromArticulations.push(prevArrowY);
                        // let oldSize = articulations[i].length;
                        articulations[i] = articulations[i].filter(
                            (e) => !removeFromArticulations.includes(e[2]),
                        );
    ...
    articulations = articulations
        .map((p) => p.map((e) => [e[0], e[1]]))
        .flat() // Get rid of info we no longer need
        ...
        .map((e) => e[0].split("--- Or ---").map((c) => [c, e[1]]))
        .flat() // If left side has OR then we need to split it into multiple entries
        .map((e) => e[1].split(" | --- Or --- | ").map((c) => [e[0], c]))
        .flat() // If right side has OR then we need to split it into multiple entries
        .map((e) => e[1].split(" | ").map((c) => [e[0], c]))
        .flat() // If right side has implicit OR (just listing a shit ton of classes, I'm pretty sure this is OR) we also need to split
        .filter(
            (e) => e[1].toLowerCase() !== "or" && e[1].toLowerCase() !== "and",
        ); // Filter out bad edge cases
...
export async function processPdf(pdfPath) {
    ...
    await new Promise((resolve, reject) => {
        ...
        pdfParser.on("pdfParser_dataError", (errData) => {
            console.log(
                `ERROR processing ${file.split("\\").slice(3).join("/")}`,
            );
...
async function main() {
    console.log(
        await processPdf(
            "C:\\Users\\HanMan\\Desktop\\Stuff\\Random Testing\\assist pdf downloading\\assist-pdfs\\Allan_Hancock_College\\Accounting__Minor_in_---26284633.pdf",
        ),
    );
}
